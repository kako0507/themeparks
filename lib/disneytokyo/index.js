...
class DisneyTokyoPark extends Park {
  ...
  FetchWaitTimes() {
    ...
    // fetch wait times HTML page
      this.FetchWaitTimesURL().then(data =>
      // parse HTML data
  ...
  FetchWaitTimesURL() {
    return this.GetAcccessToken().then(access_token => this.HTTP({
  ...
  ParseWaitTimesJSON(data) {
    ...
    for (let rideIDX = 0; rideIDX < data.length; rideIDX++) {
      ...
      const ride = {
        ...
        WaitTime: rideStatus,
        FastPass: !!((data[rideIDX].FsStatus && data[rideIDX].FsStatusflg)), // does ride support FastPass? (and does it have any left?)
  ...
  FetchOpeningTimes() {
    return new Promise(((resolve, reject) => {
      ...
      this.FetchOpeningTimesForMonth(today.format('YYYYMM')).then(() => {
        today.add(1, 'month');
        this.FetchOpeningTimesForMonth(today.format('YYYYMM')).then(() => {
          today.add(1, 'month');
          this.FetchOpeningTimesForMonth(today.format('YYYYMM')).then(() => {
          ...
          this.HTTP({
            ...
            if (resp && resp.headers && resp.headers['set-cookie'] && resp.headers['set-cookie'].length) {
              ...
              for (var i = 0, cookie_string; cookie_string = resp.headers['set-cookie'][i++];) {
                const cookie_data = cookie.parse(cookie_string);
