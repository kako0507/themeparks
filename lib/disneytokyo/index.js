  ...
  fetchRideData() {
    return this.cache.wrap('ridedata', async () => {
      ...
      body.attractions.forEach((attr) => {
        const type = attr.attractionType.id;
        if (
        ...
        const name = englishData && englishData.name !== undefined
          ? englishData.name
          : attr.nameKana;
        ...
        const rideObject = this.getRideObject({
          ...
          name,
          type,
          location: _.pick(attr, ['latitude', 'longitude']),
        });
        rideObject.fastPass = attr.fastpass;
      });
