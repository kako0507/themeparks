...
const s_disneyAPIResortID = Symbol();
const s_disneyAPIParkID = Symbol();
const s_disneyAPIParkRegion = Symbol();
const s_rideSchedules = Symbol();
const s_scheduleFilters = Symbol();
const s_rideTypes = Symbol();

// API settings
const api_accessTokenURL = 'https://authorization.go.com/token';
const api_accessTokenURLBody = 'grant_type=assertion&assertion_type=public&client_id=WDPRO-MOBILE.MDX.WDW.ANDROID-PROD';
const api_accessTokenURLMethod = 'POST';
const api_appID = 'WDW-MDX-ANDROID-3.4.1';
const api_baseURL = 'https://api.wdpro.disney.go.com/';

// use the same user-agent for all WDW park requests
const api_userAgent = random_useragent.getRandom(ua => (ua.osName == 'Android'));
...
class WaltDisneyWorldPark extends Park {
            ...
            this.GetFacilitiesData().then((facilitiesData) => {
              ...
              for (var i = 0, ride; ride = waitTimeData.entries[i++];) {
                ...
                if (this[s_rideSchedules][rideId]) {
                  const endFillDate = Moment().tz(this.Timezone).add(90, 'days');
                  for (let m = Moment().tz(this.Timezone); m.isBefore(endFillDate); m.add(1, 'day')) {
                    const rideScheduleData = this[s_rideSchedules][rideId].GetDate({
                      date: m,
                    });
                    if (rideScheduleData) {
  ...
  FetchOpeningTimes() {
    return new Promise(((resolve, reject) => {
      ...
      const rangeEnd = Moment.tz(this.Timezone).add(30, 'days').format('YYYY-MM-DD');
        ...
        this.GetAPIUrl({
          ...
          for (var i = 0, element; element = data.added[i++];) {
            ...
            facilitiesData[CleanRideID(element.id)] = {
              ...
              fastPass: element.fastPass && element.fastPass == 'true' ? true : (!!(element.fastPassPlus && element.fastPassPlus == 'true')),
            };
          }
...
function CleanRideID(ride_id) {
  const capture = regexTidyID.exec(ride_id);
