...
class AltonTowers extends MerlinPark {
   ...
   * @name AltonTowers.calendarUrl
   * @type {String}
   */
  static calendarUrl = 'https://www.altontowers.com/umbraco/api/openingtimes/GetAllAttractionOpeningTimes';
    ...
    return new Promise((resolve, reject) => {
      this.http({
        ...
        const today = moment
          ...
          .second(0);
        if (
          !calendarData.Attractions ||
          !calendarData.Attractions[0] ||
          !calendarData.Attractions[0].Attraction
        ) {
          reject(new Error('Invalid/Unknown calendar data returned'));
          return;
        }
        let parkEvents;
        // for resorts with multiple parks
        calendarData.Attractions.some((times) => {
          if (times.Attraction === 'themepark') {
            parkEvents = times.DateRanges;
            return true;
          }
          return false;
        });
        parkEvents.forEach((timeRange) => {
          const { timezone } = this.constructor;
          const endDate = moment.tz(timeRange.EndDate, 'YYYY-MM-DDTHH:mm:ss', timezone);
          if (today.isAfter(endDate, 'day') || timeRange.OpeningHours === 'Open') {
            return;
          }
          const startDate = moment.tz(timeRange.StartDate, 'YYYY-MM-DDTHH:mm:ss', timezone);
          ...
          this.schedule.setRange({
            startDate,
            endDate,
            ...this.parseOpeningTime(timeRange.OpeningHours),
          });
        });
