...
import DebugLog from './debugPrint';
...
class Park {
  ...
  constructor(options = {}) {
    ...
    if (!this.constructor.location) {
      throw new Error(`No park GeoLocation object created for ${this.constructor.name}. Please supply longitude and latitude for this park.`);
    }
   ...
   * @param {String} rideId - Ride's ID
   * @returns {Ride} ride - Existing Ride object (or null if it doesn't exist)
   */
  findRideObject(rideId) {
    if (!rideId) {
      this.log('No Ride ID supplied to findRideObject');
      return null;
    }

    if (rideId === undefined) {
      this.log('No Ride ID supplied to findRideObject', rideId);
      return null;
    }

    // check if we have this ride yet
    if (this[symbolRideIdToIdxMap][rideId] === undefined) {
      return null;
    }

    // return the already existing ride
    return this.rides[this[symbolRideIdToIdxMap][rideId]];
  }
  ...
  http(options) {
    ...
    return HTTPLib({
      ...
      timeout: options.timeout || settings.defaultHttpTimeout,
   ...
   */
  log(args) {
    return DebugLog(`${this.constructor.name}:`, ...args);
  }
}
