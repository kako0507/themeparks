      ...
      this.getRideNames().then(() => {
        this.getAPIUrl({
          ...
          rideData.rideStatuses.forEach((ride) => {
            ...
            const rideObject = this.findRideObject(ride.rideId);
  ...
  getAccessToken() {
    ...
    return this.cache.wrap('accesstoken', () => new Promise(((resolve, reject) => {
      this.http({
        ...
        if (!data) {
          reject(new Error('No response data returned for access token'));
          return;
        }
        ...
        this.log('Fetched access token', data.access_token);

        ttl = data.expires_in || 60 * 30;
  ...
  getRideNames() {
    return this.cache.wrap('rides', () => new Promise(((resolve, reject) => {
      ...
      this.getAPIUrl({
        ...
        if (!data) {
          reject(new Error('No response data recieved'));
          return;
        }
        if (!data.rides) {
