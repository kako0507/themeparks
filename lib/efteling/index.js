...
class Efteling extends Park {
  ...
  makeRequest(requestOptions) {
    ...
    return this.http({
      ...
      if (!response.data) return Promise.reject(new Error('Failed to get network response'));
      ...
      let JSONResult;
      try {
        JSONResult = JSON.parse(response.data);
        return Promise.resolve(JSONResult);
      } catch (e1) {
        // failed to parse JSON data? assume it's encrypted and decrypt it first
        let decryptedString;
        try {
          decryptedString = this.decryptString(response.data);
        } catch (e2) {
          throw new Error(`Failed to decrypt string: ${response.data}`);
        }
