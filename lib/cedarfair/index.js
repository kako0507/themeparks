  ...
  FetchOpeningTimes() {
    return new Promise(((resolve, reject) => {
      ...
      this.FetchStaticScheduleData().then((scheduleData) => {
        ...
        for (let monthIdx = 0; month = scheduleData.main.months[monthIdx++];) {
          for (let hoursIdx = 0; dayRow = month.hours[hoursIdx++];) {
            for (let dayIdx = 0; day = dayRow[dayIdx++];) {
        ...
        for (var specialHourIdx = 0, specialHours; specialHours = this[s_specialHours][specialHourIdx++];) {
          ...
          for (let specialMonthIdx = 0; month = scheduleData[specialHours].months[specialMonthIdx++];) {
            for (let specialHoursIdx = 0; dayRow = month.hours[specialHoursIdx++];) {
              for (let specialDayIdx = 0; day = dayRow[specialDayIdx++];) {
                // skip this entry if there is no day set
                if (!day.day) continue;
                // skip this entry if the class doesn't appear in the legend
...
const regex_legendTimes = /([0-9]+(?::[0-9]+)?[ap]m)\s*-\s*([0-9]+(?::[0-9]+)?[ap]m)/i;
...
function ParseOpeningLegend(legendData) {
  ...
  for (var legendIdxA = 0, legendA; legendA = legendData[legendIdxA++];) {
    for (var legendIdxB = 0, legend; legend = legendA[legendIdxB++];) {
      ...
      const times = regex_legendTimes.exec(legend.description);
